...
const segment_name = "e2e_test_none_segment"
const segment_description = "e2e_test for none segment"
const label_name = Cypress.config().none_label
const game_code = "mherosgb"
const id_type = "adid"
const aggr_value = "3"

describe('segment create test none', () => {
    beforeEach(() => {
    })
    ...
    it('label with 1 value condition', () => {
        ...
        cy.get('[data-cy=date_condition_value_0_0]').eq(1).click()  // v-select에서 2번째 선택

        // 레이블 조건 입력
        cy.get('[data-cy=add_aggr_condition_initial_0_0]').click()       // 조건 추가
        cy.get('[data-cy=aggr_value_sign_0_0_0_0]').parent().click()     // 조건 수식 변경
        cy.get('[data-cy=aggr_value_sign_value_0_0_0_0]').eq(1).click()  // v-select에서 2번째 선택
        cy.get('[data-cy=aggr_value_0_0_0_0]').click().type(aggr_value)
        ...
        cy.intercept('POST', url + 'mgmt/segment/jobs', (req) => {
            ...
            req.continue((res) => {
                ...
                expect(res.body.definitions[0].query_config.meta.segmentLabelQueryMetas[0].dataType).to.eq("NONE")
                expect(res.body.definitions[0].query_config.meta.segmentLabelQueryMetas[0].gameCodes[0]).to.eq(game_code)
                expect(res.body.definitions[0].query_config.meta.segmentLabelQueryMetas[0].startJobDate).to.eq(null)
                ...
                expect(res.body.definitions[0].query_config.meta.segmentLabelQueryMetas[0].segmentColumnInfosList[2].segmentColumnInfos[0].column).to.eq("COUNT(value)")
                ...
                expect(res.body.definitions[0].query_config.meta.segmentLabelQueryMetas[0].segmentColumnInfosList[2].segmentColumnInfos[0].conditionValue).to.eq(aggr_value)
                expect(res.body.definitions[0].query_config.meta.segmentLabelQueryMetas[0].segmentColumnInfosList[2].segmentColumnInfos[0].alias).to.eq(label_name + "_COUNT")
            })
        }).as('segmentCreate')
...
describe('delete none segment', () => {
    var segment_id = 0
    it('delete none label',  () => {
        cy.visit('/')
        cy.wait(5000)
        cy.intercept('POST', url + 'mgmt/segment/jobs', (req) => {
            console.log(req)
