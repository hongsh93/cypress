...
const segment_name = "e2e_test_json_string_segment"
const segment_description = "e2e_test for json_string segment"
const label_name = Cypress.config().json_string_label
const game_code = "mherosgb"
const id_type = "adid"
const condition_key = "test_key"
const condition_value  = "abc"

describe('segment create test json_string', () => {
    beforeEach(() => {
    })
    ...
    it('label with 1 value condition', () => {
        ...
        cy.intercept('POST', url + 'mgmt/segment/jobs', (req) => {
            ...
            req.continue((res) => {
                ...
                expect(res.body.definitions[0].query_config.meta.segmentLabelQueryMetas[0].secondaryDataType).to.eq("STRING")
                ...
                expect(res.body.definitions[0].query_config.meta.segmentLabelQueryMetas[0].segmentColumnInfosList[2].segmentColumnInfos[0].column).to.eq("LOWER(REGEXP_REPLACE(JSON_EXTRACT(value, '$." + condition_key + "'), '\"', ''))")
...
describe('delete json_string segment', () => {
    var segment_id = 0
    it('delete json_string label',  () => {
        cy.visit('/')
        cy.wait(5000)
        cy.intercept('POST', url + 'mgmt/segment/jobs', (req) => {
            console.log(req)
