...
const segment_name = "e2e_test_json_number_segment"
const segment_description = "e2e_test for json_number segment"
const label_name = Cypress.config().json_number_label
const game_code = "mherosgb"
const id_type = "adid"
const condition_key = "test_key"
const condition_value  = "123"
const aggr_condition_value1  = "2"
const aggr_condition_value2  = "3"

describe('segment create test json_number', () => {
    beforeEach(() => {
    })

    it('label with 1 value and 2 and aggr condition', () => {
        cy.visit('/')
        cy.wait(5000)
        ...
        cy.get('[data-cy=aggr_value_0_0_0_0]').click().type(aggr_condition_value1)

        cy.get('[data-cy=add_aggr_condition_0_0_0]').click()       // 조건 추가
        cy.get('[data-cy=add_and_aggr_condition_0_0_0]').click()
        cy.get('[data-cy=aggr_aggregation_0_0_0_1]').parent().click()     // 조건 수식 변경
        cy.get('[data-cy=aggr_aggregation_value_0_0_0_1]').eq(2).click()  // v-select에서 2번째 선택
        cy.get('[data-cy=aggr_value_0_0_0_1]').click().type(aggr_condition_value2)
        ...
        cy.intercept('POST', url + 'mgmt/segment/jobs', (req) => {
            ...
            req.continue((res) => {
                ...
                expect(res.body.definitions[0].query_config.meta.segmentLabelQueryMetas[0].secondaryDataType).to.eq("NUMBER")
                ...
                expect(res.body.definitions[0].query_config.meta.segmentLabelQueryMetas[0].segmentColumnInfosList[2].segmentColumnInfos[0].column).to.eq("SAFE_CAST(REGEXP_REPLACE(JSON_EXTRACT(value, '$." + condition_key + "'), '\"', '') AS FLOAT64)")
                ...
                expect(res.body.definitions[0].query_config.meta.segmentLabelQueryMetas[0].segmentColumnInfosList[3].segmentColumnInfos[0].column).to.eq("SAFE_CAST(userdata_hub_common.CLBS_JSON_EXTRACT_AGGR(value, 'sum', '') AS FLOAT64)")
                ...
                expect(res.body.definitions[0].query_config.meta.segmentLabelQueryMetas[0].segmentColumnInfosList[3].segmentColumnInfos[0].conditionValue).to.eq(aggr_condition_value1)
                expect(res.body.definitions[0].query_config.meta.segmentLabelQueryMetas[0].segmentColumnInfosList[3].segmentColumnInfos[0].aggrOp).to.eq("SUM")
                expect(res.body.definitions[0].query_config.meta.segmentLabelQueryMetas[0].segmentColumnInfosList[3].segmentColumnInfos[0].alias).to.eq(label_name + "_SUM")

                expect(res.body.definitions[0].query_config.meta.segmentLabelQueryMetas[0].segmentColumnInfosList[3].segmentColumnInfos[1].column).to.eq("SAFE_CAST(userdata_hub_common.CLBS_JSON_EXTRACT_AGGR(value, 'avg', '') AS FLOAT64)")
                expect(res.body.definitions[0].query_config.meta.segmentLabelQueryMetas[0].segmentColumnInfosList[3].segmentColumnInfos[1].conditionOp).to.eq("=")
                expect(res.body.definitions[0].query_config.meta.segmentLabelQueryMetas[0].segmentColumnInfosList[3].segmentColumnInfos[1].conditionValue).to.eq(aggr_condition_value2)
                expect(res.body.definitions[0].query_config.meta.segmentLabelQueryMetas[0].segmentColumnInfosList[3].segmentColumnInfos[1].aggrOp).to.eq("AVG")
                expect(res.body.definitions[0].query_config.meta.segmentLabelQueryMetas[0].segmentColumnInfosList[3].segmentColumnInfos[1].alias).to.eq(label_name + "_AVG")
            })
        }).as('segmentCreate')
...
describe('delete json_number segment', () => {
    var segment_id = 0
    it('delete json_number label',  () => {
        cy.visit('/')
        cy.wait(5000)
        cy.intercept('POST', url + 'mgmt/segment/jobs', (req) => {
            console.log(req)
